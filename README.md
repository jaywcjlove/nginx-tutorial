...
<!-- TOC -->autoauto- [安装](#安装)auto  - [安装依赖](#安装依赖)auto  - [下载](#下载)auto  - [编译安装](#编译安装)auto  - [nginx测试](#nginx测试)auto  - [设置全局nginx命令](#设置全局nginx命令)auto- [Mac 安装](#mac-安装)auto  - [安装nginx](#安装nginx)auto  - [启动服务](#启动服务)auto- [开机自启动](#开机自启动)auto- [运维](#运维)auto  - [服务管理](#服务管理)auto  - [重启服务防火墙报错解决](#重启服务防火墙报错解决)auto- [nginx卸载](#nginx卸载)auto- [参数说明](#参数说明)auto- [配置](#配置)auto  - [常用正则](#常用正则)auto  - [全局变量](#全局变量)auto  - [符号参考](#符号参考)auto  - [配置文件](#配置文件)auto  - [内置预定义变量](#内置预定义变量)auto  - [反向代理](#反向代理)auto  - [负载均衡](#负载均衡)auto    - [RR](#rr)auto    - [权重](#权重)auto    - [ip_hash](#ip_hash)auto    - [fair](#fair)auto    - [url_hash](#url_hash)auto  - [屏蔽ip](#屏蔽ip)auto- [第三方模块安装方法](#第三方模块安装方法)auto- [重定向](#重定向)auto  - [重定向整个网站](#重定向整个网站)auto  - [重定向单页](#重定向单页)auto  - [重定向整个子路径](#重定向整个子路径)auto- [性能](#性能)auto  - [内容缓存](#内容缓存)auto  - [Gzip压缩](#gzip压缩)auto  - [打开文件缓存](#打开文件缓存)auto  - [SSL缓存](#ssl缓存)auto  - [上游Keepalive](#上游keepalive)auto  - [监控](#监控)auto- [常见使用场景](#常见使用场景)auto  - [跨域问题](#跨域问题)auto  - [跳转到带www的域上面](#跳转到带www的域上面)auto  - [代理转发](#代理转发)auto  - [监控状态信息](#监控状态信息)auto  - [代理转发连接替换](#代理转发连接替换)auto  - [ssl配置](#ssl配置)auto  - [强制将http重定向到https](#强制将http重定向到https)auto  - [两个虚拟主机](#两个虚拟主机)auto  - [虚拟主机标准配置](#虚拟主机标准配置)auto  - [爬虫过滤](#爬虫过滤)auto  - [防盗链](#防盗链)auto  - [虚拟目录配置](#虚拟目录配置)auto  - [防盗图配置](#防盗图配置)auto  - [屏蔽.git等文件](#屏蔽git等文件)auto  - [域名路径加不加需要都能正常访问](#域名路径加不加需要都能正常访问)auto- [错误问题](#错误问题)auto- [Nginx 模块](#nginx-模块)auto- [精品文章参考](#精品文章参考)autoauto<!-- /TOC -->
